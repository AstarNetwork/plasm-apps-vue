...
import { QueueTx, QueueTxMessageSetStatus, QueueTxStatus, QueueTxResult } from '@/types/Status';
...
export default function useSendTx() {
  ...
  const sendRpc = async (api: ApiPromise, { id, rpc, values = [] }: QueueTx): Promise<void> => {
    console.log('r', rpc);
    if (rpc) {
  ...
  async function signAndSend(currentItem: QueueTx, tx: SubmittableExtrinsic<'promise'>, pairOrAddress: KeyringPair | string, options: Partial<SignerOptions>): Promise<void> {
    ...
    try {
      ...
      const unsubscribe = await tx.send(handleTxResults('signAndSend', currentItem, (): void => {
        console.log('sent tx')
        unsubscribe();
      }));
    } catch (error) {
  ...
  async function extractParams(api: ApiPromise, address: string, options: Partial<SignerOptions>): Promise<['qr' | 'signing', string, Partial<SignerOptions>]> {
    ...
    } else if (isInjected) { // polkadot extension
  ...
  const onSend = async (api: ApiPromise, currentItem: QueueTx, senderInfo: AddressProxy): Promise<void> => {
